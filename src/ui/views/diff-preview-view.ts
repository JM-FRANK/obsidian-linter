...
export const diffPreviewViewType = 'linter-frank-diff-preview';
...
export class DiffPreviewView extends ItemView {
  ...
  async onOpen() {
    ...
    this.contentEl.addClass('linter-frank-diff-preview-view');
    this.render();
  }
  ...
  private render() {
    ...
    if (!this.previewState) {
      this.contentEl.createDiv({
        cls: 'linter-frank-diff-preview-empty',
        text: getTextInLanguage('notice-text.no-preview-changes'),
      });
      return;
    }

    const headerEl = this.contentEl.createDiv('linter-frank-diff-preview-header');
    const titleGroupEl = headerEl.createDiv('linter-frank-diff-preview-title-group');
    ...
    if (!hasChanges) {
      titleGroupEl.createDiv({
        cls: 'linter-frank-diff-summary',
        text: getTextInLanguage('notice-text.no-preview-changes'),
      });
    }

    const actionsEl = headerEl.createDiv('linter-frank-diff-preview-actions');
    ...
    this.bodyEl = this.contentEl.createDiv('linter-frank-diff-preview-body');
    if (!hasChanges) {
      return;
    }
  ...
  private renderDiff() {
    ...
    const diffEl = this.bodyEl.createDiv({cls: 'linter-frank-diff'});
  ...
  private renderDiffLines(diffEl: HTMLDivElement, lines: Array<DiffLine | {skipped: number}>) {
    for (const line of lines) {
      ...
      lineEl.createSpan({cls: 'linter-frank-diff-marker', text: this.diffPrefix(line.operation)});
      lineEl.createSpan({cls: 'linter-frank-diff-text', text: line.text});
    }
  }

  private diffClass(operation: number): string {
    if (operation === DiffMatchPatch.DIFF_INSERT) {
      return 'linter-frank-diff-line linter-frank-diff-added';
    } else if (operation === DiffMatchPatch.DIFF_DELETE) {
      return 'linter-frank-diff-line linter-frank-diff-removed';
    }

    return 'linter-frank-diff-line linter-frank-diff-context';
  }
